...
async def cmd_help(message: types.Message):
    help_text = (
        ...
        "📊 Недельный отчёт — генерируется автоматически (пн 09:30)\n"
        "   Или по кнопке из /dashboard\n"
        "⚡ /smite ID 20 Причина — наложить штраф\n"
        "🌴 /vacation ID start_date end_date — отпуск (отключает штрафы)\n\n"
        ...
        "⏸ <b>Кнопка «Отсрочка»</b> — Продлить таймер на <b>+4 часа</b>\n"
        "   → Доступна после взятия квеста, можно использовать несколько раз\n"
